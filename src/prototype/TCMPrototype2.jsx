...
function ConsultScreen() {
  ...
  const allergyOptions = ["无", "花粉", "海鲜", "药物", "其他"];
  ...
  const profileHistorySummary = useMemo(() => {
    const tags = profileData.historyTags.filter((item) => item !== "其他");
    if (profileData.historyTags.includes("其他") && profileData.historyDetail.trim()) {
      tags.push(profileData.historyDetail.trim());
    }
    return tags.join("、") || "待补充";
  }, [profileData.historyDetail, profileData.historyTags]);

  const profileAllergySummary = useMemo(() => {
    const tags = profileData.allergyTags.filter((item) => item !== "其他");
    if (profileData.allergyTags.includes("其他") && profileData.allergyDetail.trim()) {
      tags.push(profileData.allergyDetail.trim());
    }
    return tags.join("、") || "待补充";
  }, [profileData.allergyDetail, profileData.allergyTags]);

  const profileBasicSummary = useMemo(() => {
    const parts = [`${profileData.gender}`, `${profileData.age}岁`];
    if (profileBmi !== "--") parts.push(`BMI ${profileBmi}`);
    return parts.join(" · ");
  }, [profileData.age, profileData.gender, profileBmi]);

  const profileRecordPreview = useMemo(() => {
    const historyCount =
      profileData.historyTags.filter((item) => item !== "无重大既往病史").length +
      (profileData.historyDetail.trim() ? 1 : 0);
    const allergyCount =
      profileData.allergyTags.filter((item) => item !== "无").length +
      (profileData.allergyDetail.trim() ? 1 : 0);

    if (!historyCount && !allergyCount) return "既往病史与过敏信息";
    if (!historyCount) return `已记录 ${allergyCount} 项过敏信息`;
    if (!allergyCount) return `已记录 ${historyCount} 项既往情况`;
    return `已记录 ${historyCount} 项既往情况 · ${allergyCount} 项过敏信息`;
  }, [
    profileData.allergyDetail,
    profileData.allergyTags,
    profileData.historyDetail,
    profileData.historyTags,
  ]);
  ...
  const profileComplete = Boolean(
    profileData.gender &&
      ...
      profileData.historyTags.length &&
      profileData.allergyTags.length &&
      (!profileData.allergyTags.includes("其他") || profileData.allergyDetail.trim()) &&
      (!profileData.historyTags.includes("其他") || profileData.historyDetail.trim())
  );
            ...
            {/* <Card className={sectionCard}>
              ...
              <CardContent className="space-y-4">
                <div className="rounded-2xl border border-emerald-100 bg-gradient-to-br from-emerald-50 to-white p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3">
                      <div className="rounded-2xl bg-white p-2.5 shadow-sm ring-1 ring-emerald-100">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                    <div className="text-[11px] text-slate-500">最近同步</div>
                    <div className="mt-1 text-[15px] font-semibold tracking-tight text-slate-900">
                      2026/04/14 10:32
                    </div>
                  </div>
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                    <div className="text-[11px] text-slate-500">数据来源</div>
                    <div className="mt-1 text-[15px] font-semibold tracking-tight text-slate-900">
                      手指医生设备
                    </div>
                  </div>
                </div>
                <div className="rounded-2xl border border-dashed border-slate-200 bg-white px-4 py-3 text-[12px] text-slate-500">
                  检测结果已自动归档，可在后续辨证与方案生成中联动使用
                </div>
              </CardContent>
            </Card> */}
              ...
              <CardContent className="space-y-4 text-sm">
                <div className="rounded-2xl border p-4 bg-white">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <div className="text-sm font-medium">性别</div>
                      <div className="text-xs text-muted-foreground mt-1">
                        低频信息，适合快捷切换
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-2xl border p-4 bg-white">
                    <div className="flex items-center justify-between gap-3">
                      <div className="text-sm font-medium">年龄</div>
                      <Badge variant="secondary" className={fieldBadgeClass}>
                        数字
                      </Badge>
                    </div>
                    <Input
                      className="mt-3 h-11 rounded-xl px-4"
                      inputMode="numeric"
                      placeholder="请输入年龄"
                  ...
                  <div className="rounded-2xl border p-4 bg-white">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <Badge variant="secondary" className={badgeDoneClass}>
                        自动计算
                      </Badge>
                    </div>
                    <div className="mt-3 rounded-xl bg-slate-50 px-4 py-3 text-base font-semibold text-slate-900">
                      {profileBmi}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-2xl border p-4 bg-white">
                    <div className="flex items-center justify-between gap-3">
                      <div className="text-sm font-medium">身高(cm)</div>
                      <Badge variant="secondary" className={fieldBadgeClass}>
                        数字
                      </Badge>
                    </div>
                ...
                <div className="rounded-2xl border p-4 bg-white">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <div className="text-sm font-medium">过敏史</div>
                      <div className="text-xs text-muted-foreground mt-1">
                        适合标签式快速点选
                      </div>
                    </div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {allergyOptions.map((option) => (
                      <button
                        ...
                        type="button"
                        className={
                          profileData.allergyTags.includes(option)
                            ? badgeSelectedClass
                            : badgeIdleClass
                        }
                        onClick={() =>
                          setProfileData((prev) => {
                            const hasOption = prev.allergyTags.includes(option);

                            if (option === "无") {
                              return {
                                ...prev,
                                allergyTags: hasOption ? [] : ["无"],
                                allergyDetail: "",
                              };
                            }

                            const nextTags = hasOption
                              ? prev.allergyTags.filter((item) => item !== option)
                              : [
                                  ...prev.allergyTags.filter((item) => item !== "无"),
                                  option,
                                ];

                            return {
                              ...prev,
                              allergyTags: nextTags,
                              allergyDetail:
                                option === "其他" || nextTags.includes("其他")
                                  ? prev.allergyDetail
                                  : "",
                            };
                          })
                  ...
                  {profileData.allergyTags.includes("其他") ? (
                    <Input
                      className="mt-3 h-11 rounded-xl px-4"
                      placeholder="请补充具体过敏信息"
                      value={profileData.allergyDetail}
                      onChange={(e) =>
                        setProfileData((prev) => ({
                          ...prev,
                          allergyDetail: e.target.value,
                        }))
                      }
                    />
                  ) : null}
                ...
                <div className="rounded-2xl border p-4 bg-white">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <div className="text-sm font-medium">既往病史</div>
                      <div className="text-xs text-muted-foreground mt-1">
                        支持多选标签，补充信息单独输入
                      </div>
                    </div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {historyOptions.map((option) => (
                      <button
                        ...
                        type="button"
                        className={
                          profileData.historyTags.includes(option)
                            ? badgeSelectedClass
                            : badgeIdleClass
                        }
                        onClick={() =>
                          setProfileData((prev) => {
                            const hasOption = prev.historyTags.includes(option);

                            if (option === "无重大既往病史") {
                              return {
                                ...prev,
                                historyTags: hasOption ? [] : [option],
                                historyDetail: "",
                              };
                            }

                            const nextTags = hasOption
                              ? prev.historyTags.filter((item) => item !== option)
                              : [
                                  ...prev.historyTags.filter(
                                    (item) => item !== "无重大既往病史"
                                  ),
                                  option,
                                ];
                  ...
                  {profileData.historyTags.includes("其他") ? (
                    <textarea
                      ...
                      rows={3}
                      placeholder="请补充既往病史、手术史或长期健康问题"
                      value={profileData.historyDetail}
                      onChange={(e) =>
                        setProfileData((prev) => ({
                          ...prev,
                          historyDetail: e.target.value,
                        }))
                      }
                    />
                  ) : null}
        ...
        <div className="p-4 space-y-4">
          ...
          <Card className={sectionCard}>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3 text-xs">
                {syndromeScores.map(([label, value]) => (
                  <div
                    key={label}
          ...
          <Card className={sectionCard}>
            ...
            <CardContent className="space-y-3">
              {diagnosisDataSections.map((section) => (
                <div
                  key={section.key}
                    ...
                    {section.key === "tongue" ? (
                      ...
                      >
                        <div className="flex items-center justify-between px-3 pt-3">
                          <div className="text-[11px] text-slate-500">舌象图像</div>
      ...
      <div className="p-4 space-y-4">
        ...
        <Card className={sectionCard}>
          ...
          <CardContent className="space-y-3">
            <button
              type="button"
              onClick={() => setPlanView("diagnosis-detail")}
...
function TherapyScreen({ prefill, clearPrefill }) {
  ...
  const submitAppointment = (conflictId = null) => {
    const nextAppointment = buildAppointmentRecord();
    if (!nextAppointment) return;
    setAppointments((prev) => {
      const nextList = prev.map((item) =>
        item.id === conflictId ? { ...item, status: "cancelled" } : item
      );
      return [nextAppointment, ...nextList];
    });
    setShowBookingSheet(false);
  ...
  const handleSubmitAppointment = () => {
    if (!hasBookingBasics || !selectedInstitutionInfo || !selectedProjectInfo) return;

    const conflict = appointments.find(
      (item) => item.status !== "cancelled" && item.time === selectedTime
    );
  ...
  const isPastTimeSlot = (dateValue, slot) => {
    if (!dateValue) return false;
    const option = dateOptions.find(
      (item) => `${item.label} ${item.sub}` === dateValue
    );
    if (!option || option.label !== "今天") return false;
  ...
  useEffect(() => {
    if (!selectedDate || !isPastTimeSlot(selectedDate, selectedSlot)) return;
    const nextAvailableSlot = timeSlots.find(
      (slot) => !isPastTimeSlot(selectedDate, slot)
    );
  ...
  return (
    <div className="bg-slate-50 min-h-full pb-24">
      ...
      <div className="p-4 space-y-4">
        ...
        {showConflictDialog && conflictAppointment ? (
          <>
            <div
            ...
            />
            <div className="fixed inset-x-4 top-1/2 z-[80] -translate-y-1/2 rounded-3xl bg-white p-5 shadow-2xl">
              <div className="text-base font-semibold text-slate-900">
                检测到冲突预约
              </div>
              <div className="mt-2 text-sm leading-6 text-slate-600">
                当前时段已存在预约项目。确认后将自动取消之前的预约，并提交新的预约。
              </div>

              <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm">
                <div className="font-medium text-slate-900">
                  原预约：{conflictAppointment.project}
                </div>
                <div className="mt-1 text-slate-500">
                  {conflictAppointment.institution}
                </div>
                <div className="mt-2 text-[12px] text-slate-500">
                  预约时间：{conflictAppointment.time}
                </div>
              </div>

              <div className="mt-3 rounded-2xl border border-emerald-100 bg-emerald-50 p-4 text-sm">
                <div className="font-medium text-slate-900">
                  新预约：{selectedProjectInfo?.name || "未选择项目"}
                </div>
                <div className="mt-1 text-slate-500">
                  {selectedInstitutionInfo?.name || "未选择机构"}
                </div>
                <div className="mt-2 text-[12px] text-slate-500">
                  预约时间：{selectedTime || "未选择时间"}
                </div>
              </div>
              ...
              <div className="mt-5 flex gap-3">
                ...
                <Button
                  type="button"
                  className="flex-1 rounded-2xl"
                  onClick={() => submitAppointment(conflictAppointment.id)}
                >
                  覆盖并提交
                </Button>
              </div>
